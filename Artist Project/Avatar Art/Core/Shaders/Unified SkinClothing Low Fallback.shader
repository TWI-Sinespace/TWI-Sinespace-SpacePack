Shader "Sine Wave/Skin/Unified Clothing and Skin (Mobile)" {
Properties {
	_Color ("Main Color", Color) = (1,1,1,1)
	_SpecColor ("Specular Color", Color) = (1,1,1,1)
	_Shininess ("Shininess", float) = 0.078125
	_MainTex ("Base (RGB) Alpha Test (A)", 2D) = "white" {}
	_MetallicGlossMap ("Spec (RGB) Gloss (A)", 2D) = "white" {}
...
CGPROGRAM
#pragma surface surf StandardSpecular nolightmap noforwardadd
...
void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
	fixed4 spec =  tex2D(_MetallicGlossMap, IN.uv_MetallicGlossMap) * _SpecColor;
