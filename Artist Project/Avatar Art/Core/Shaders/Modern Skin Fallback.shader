Shader "Sine Wave/Skin/Modern Skin Fallback" {
Properties {
...
//#pragma target 3.0
#pragma surface surf StandardSpecular2 nolightmap keepalpha
...
void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
	...
	o.Alpha = 0;
	o.Specular = spec.rgb + ((1.0 - IN.color.g) * tex.rgb) * _Shininess * _SpecColor * 0.6f;// 0.95 * texMod.rgb * spec.a * _Shininess;// _SpecColor;//spec.rgb * 8.1;
	...
	o.Normal = lerp(normalize(normal), float3(0, 0, 1), IN.color.g);
	...
	//#else 
	o.Smoothness = (saturate((spec.a * 8)) * 0.4) * 0.5;// lerp(_Smoothness * lerp(1.0, tex2.r, _DetailPower), 0.6, IN.color.g) * saturate(spec.a * 8);// *tex2.r * tex3.r;//0.6892;
	_CornerSmoothness = lerp(_CornerSmoothness, 1.0, IN.color.g);
	o.Occlusion = 1.0f;//lerp(tex2.r * tex3.r, 1.0, ) ;//.5;//2.5; //148 / 0.892;
	//#endif
...
FallBack "Sine Wave/Skin/Unified Clothing and Skin"
}
